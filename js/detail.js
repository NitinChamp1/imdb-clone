...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const item = [...MOVIES, ...TV_SHOWS].find(m => m.id === id);
  if (!item) {
  ...
  const similar = (type === 'movie' ? MOVIES : TV_SHOWS)
      ...
      <div class="container-fluid px-4 position-relative" style="z-index:2; padding-top:80px; padding-bottom:40px;">
        <nav aria-label="breadcrumb" class="mb-3">
          <ol class="breadcrumb mb-0">
            <li class="breadcrumb-item"><a href="index.html" class="text-muted text-decoration-none">Home</a></li>
            <li class="breadcrumb-item"><a href="${type === 'tv' ? 'tvshows.html' : 'movies.html'}" class="text-muted text-decoration-none">${type === 'tv' ? 'TV Shows' : 'Movies'}</a></li>
          ...
          <div class="flex-grow-1">
            ...
            <div class="d-flex flex-wrap gap-3 mb-3">
              ...
              <div class="rating-box">
                <div class="text-muted small mb-1">IMDb Rating</div>
                <div class="rating-value"><i class="bi bi-star-fill text-warning me-1" style="font-size:1.4rem;"></i>${item.rating}</div>
                <div class="rating-count">${item.votes} votes</div>
              </div>
              <!-- User Rating Box -->
              <div class="rating-box">
                <div class="text-muted small mb-2">Your Rating</div>
                ...
                <div class="rating-count mt-1" id="userRatingText">${userRating ? `You rated: ${userRating}/10` : 'Click to rate'}</div>
              </div>
              ${item.ranked ? `
              <div class="rating-box">
                <div class="text-muted small mb-1">IMDb Rank</div>
                <div class="rating-value text-warning">#${item.ranked}</div>
                <div class="rating-count">Top Movies</div>
              </div>` : ''}
            </div>
            <div class="d-flex gap-2 flex-wrap">
              <button class="btn btn-warning rounded-pill px-4 fw-700">
        ...
        <div class="row g-4">
          ...
          <div class="col-lg-8">
            ...
            <div class="review-card mt-4">
              ...
              <textarea class="form-control bg-transparent border-secondary text-white mb-3" id="reviewText" rows="3" placeholder="Share your thoughts about this movie..." style="resize:none;"></textarea>
              <div class="d-flex justify-content-between align-items-center">
                <div class="text-muted small">Your rating: <span class="text-warning" id="reviewRatingDisplay">${userRating ? `${userRating}/10` : 'Rate above'}</span></div>
...
function generateMockReviews(item) {
  ...
  return reviews.map(r => `
    <div class="review-card mb-3">
      ...
      <p class="text-muted small mb-2 lh-base">${r.text}</p>
...
function submitReview() {
  ...
  const newReview = `
    <div class="review-card mb-3 animate-in">
      <div class="d-flex justify-content-between align-items-center mb-2">
        <div class="d-flex align-items-center gap-2">
          <div style="width:36px;height:36px;background:linear-gradient(135deg,#28a745,#1a7a34);border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:700;color:#fff;font-size:0.85rem;">Y</div>
          <div>
            <div class="fw-600 small text-white">You</div>
            <div class="text-muted" style="font-size:0.7rem;">Just now</div>
          </div>
        </div>
        <span class="badge bg-success">New</span>
      </div>
      <p class="text-muted small mb-0 lh-base">${text}</p>
    </div>`;
