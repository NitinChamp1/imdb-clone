...
function renderWatchlistPage() {
  ...
  if (statsBar) {
    ...
    document.getElementById('statsCards').innerHTML = `
      ...
      <div class="col-6 col-md-3">
        <div class="rating-box text-center w-100">
          <div class="rating-value text-info">${list.filter(i => (i.type || 'movie') === 'movie').length}</div>
          <div class="rating-count">Movies</div>
        </div>
      </div>
      <div class="col-6 col-md-3">
        <div class="rating-box text-center w-100">
          <div class="rating-value text-success">${list.filter(i => i.type === 'tv').length}</div>
...
function renderSortedList(list, sort) {
  ...
  container.innerHTML = sorted.map((item, i) => {
    ...
    const addedDate = item.addedAt ? new Date(item.addedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : 'Unknown Date';
    return `
      <div class="watchlist-item mb-3 animate-in" style="animation-delay:${i * 0.05}s;" id="wl-item-${item.id}">
        <img class="watchlist-poster" src="${item.poster}" alt="${item.title}" style="height:130px;cursor:pointer;" 
          onclick="goToDetail(${item.id}, '${itemType}')"
          onerror="this.src='https://via.placeholder.com/90x130/1e1e1e/888?text=?'"/>
        <div class="watchlist-info flex-grow-1">
          <div class="d-flex justify-content-between align-items-start flex-wrap gap-2">
            <div style="cursor:pointer;" onclick="goToDetail(${item.id}, '${itemType}')">
              <div class="d-flex align-items-center gap-2 mb-1 flex-wrap">
                <h5 class="watchlist-title mb-0">${item.title}</h5>
                ${itemType === 'tv' ? '<span class="badge bg-info text-dark" style="font-size:0.65rem;">TV</span>' : '<span class="badge bg-secondary" style="font-size:0.65rem;">Movie</span>'}
              </div>
