...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (category) {
    let data;
    let titleText = 'Browse TV Shows';
    
    // Hide the featured/top sections if filtering by category
    const featuredRow = document.getElementById('featuredTVRow');
    if (featuredRow) featuredRow.parentElement.parentElement.style.display = 'none';
    const top5Row = document.getElementById('top5TV');
    if (top5Row) top5Row.parentElement.parentElement.style.display = 'none';
    
    if (category === 'airing_today') {
      titleText = 'Airing Today';
      data = await TMDB.getAiringToday();
    } else if (category === 'on_tv') {
      titleText = 'On TV';
      data = await TMDB.getPopularTV(1); 
    } else if (category === 'top_rated') {
      titleText = 'Top Rated TV Shows';
      data = await TMDB.getTopRatedTV(1);
    } else {
      titleText = 'Popular TV Shows';
      data = await TMDB.getPopularTV(1);
    }
    ...
    if (data && data.results) {
      const results = data.results.map(m => normalizeTMDb(m, 'tv'));
      TV_SHOWS.length = 0;
      TV_SHOWS.push(...results);
    }
  } else {
    // Default load popular
    const data = await TMDB.getPopularTV(1);
    if (data && data.results) {
      const results = data.results.map(m => normalizeTMDb(m, 'tv'));
      TV_SHOWS.length = 0;
      TV_SHOWS.push(...results);
